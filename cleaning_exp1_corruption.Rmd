...
```{r Loading data}
Q <- read.csv("./raw-data/raw-data_questionnaire_exp1_corruption.csv", stringsAsFactors = FALSE)
...
## Tidying
CorrCollaborative <- CorrCollaborative %>% 
...
CorrIndividual$Report[CorrIndividual$Report == "22458" & CorrIndividual$group == 19] <- "22689"
...
CorrCollaborative <- CorrCollaborative[(CorrCollaborative$group == 17 | CorrCollaborative$group == 18 |
                         CorrCollaborative$group == 19 | CorrCollaborative$group == 20 |
                         CorrCollaborative$group == 33 | CorrCollaborative$group == 35 |
                         CorrCollaborative$group == 37 | CorrCollaborative$group == 39 |
                         CorrCollaborative$group == 41 | CorrCollaborative$group == 44 |
                         CorrCollaborative$group == 46 | CorrCollaborative$group == 47 |
                         CorrCollaborative$group == 49),]

CorrIndividual <- CorrIndividual[(CorrIndividual$group == 17 | CorrIndividual$group == 18 |
                          CorrIndividual$group == 19 | CorrIndividual$group == 20 |
                          CorrIndividual$group == 33 | CorrIndividual$group == 35 |
                          CorrIndividual$group == 37 | CorrIndividual$group == 39 |
                          CorrIndividual$group == 41 | CorrIndividual$group == 44 |
                          CorrIndividual$group == 46 | CorrIndividual$group == 47 |
                          CorrIndividual$group == 49),]
...
CorrIndividual <- 
  CorrIndividual %>% filter(Report != "b" & Report != "d") %>%
...
Ids <- CorrIndividual[(CorrIndividual$Report > 7),] %>% 
  dplyr::select(Report, group, subject) %>%
  mutate(ID.number = Report,
         Report = NULL)
#nrow(Ids)
#[1] 106

## Remove participant ID observations from main dataset,
# seeing as we can figure this out easily by matching subject no + group
CorrIndividual <- 
  CorrIndividual %>% 
  filter(round != "pid") %>%
...
## Now we bind by row, such that all the data is in the same format, which is why Corr_Ind
...
FullData$PlayerNo <- ifelse(FullData$subject == 1, "First Mover" , 
                            ifelse(FullData$subject == 3, "First Mover",
                                   ifelse(FullData$subject == 5, "First Mover", 
                                          ifelse(FullData$subject == 7, "First Mover",
                                                 ifelse(FullData$subject == 9, "First Mover",
                                                        ifelse(FullData$subject == 11, "First Mover", "Second Mover"))))))

FullData$round <- as.numeric(FullData$round)
#labelling the parts
FullData$Condition <- ifelse(FullData$round >= 300, "Practice",
                             ifelse(FullData$round >= 200, "Post-Collaborative",
                                    ifelse(FullData$round >= 100, "Pre-Collaborative",
...
FullData$HighEffort <- ifelse(FullData$round <= 9, 0, 
                             ifelse(FullData$round <= 18, 1,
                                    ifelse(FullData$round <= 27, 0,
                                           ifelse(FullData$round <= 36, 1, NA))))
FullData$LowEffort <- ifelse(FullData$round <= 9, 1,
                             ifelse(FullData$round <= 18, 0,
                                    ifelse(FullData$round <= 27, 1,
                                           ifelse(FullData$round <= 36, 0, NA))))

FullData$Order <- ifelse(FullData$group == 17 | FullData$group == 18 |
                             FullData$group == 20 | FullData$group == 35 | 
                             FullData$group == 39 | FullData$group == 47 | 
                             FullData$group == 49, "Mirror", "Regular")
FullData$Honourcode <- ifelse(FullData$group == 37 | FullData$group == 39,
                               1, 0)

FullData$ShortPartnership <- ifelse(FullData$round <= 18  & FullData$Order == "Regular", 1,
                             ifelse(FullData$round > 18  & FullData$Order == "Mirror", 1, 0))

FullData$LongPartnership <- ifelse(FullData$round > 18  & FullData$Order == "Regular", 1,
                            ifelse(FullData$round <= 18  & FullData$Order == "Mirror", 1, 0))

##Find the doubles
FullData$Double <- ifelse(FullData$Condition == "Collaborative", duplicated(FullData[c(1,4,5,7)]) | 
                             duplicated(FullData[c(1,4,5,7)], fromLast = TRUE), NA) 
FullData$Double <- ifelse(FullData$Double == TRUE, 1, 
...
## Order Q such that its ordering fits Behaviour
Q <- Q[ order(match(Q$id, FullData$ID.number)), ]

## Create dataframes for psychological traits
H.Hexaco <- data.frame(Q$id, Q$Q37, Q$Q61, Q$Q85, Q$Q43, Q$Q67, Q$Q91, Q$Q49, Q$Q73, Q$Q55, Q$Q79, stringsAsFactors = FALSE)
H.Hexaco <- data.matrix(H.Hexaco)
E.Hexaco <- data.frame(Q$id, Q$Q36, Q$Q60, Q$Q84, Q$Q42, Q$Q66, Q$Q48, Q$Q72, Q$Q54, Q$Q78, Q$Q90, stringsAsFactors = FALSE)
E.Hexaco <- data.matrix(E.Hexaco)
X.Hexaco <- data.frame(Q$id, Q$Q35, Q$Q59, Q$Q83, Q$Q41, Q$Q65, Q$Q89, Q$Q47, Q$Q71, Q$Q53, Q$Q77, stringsAsFactors = FALSE)
X.Hexaco <- data.matrix(X.Hexaco)
A.Hexaco <- data.frame(Q$id, Q$Q34, Q$Q58, Q$Q40, Q$Q64, Q$Q82, Q$Q46, Q$Q70, Q$Q88, Q$Q52, Q$Q76, stringsAsFactors = FALSE)
A.Hexaco <- data.matrix(A.Hexaco)
C.Hexaco <- data.frame(Q$id, Q$Q33, Q$Q57, Q$Q39, Q$Q63, Q$Q45, Q$Q69, Q$Q81, Q$Q51, Q$Q75, Q$Q87, stringsAsFactors = FALSE)
C.Hexaco <- data.matrix(C.Hexaco)
O.Hexaco <- data.frame(Q$id, Q$Q32, Q$Q56, Q$Q38, Q$Q62, Q$Q44, Q$Q68, Q$Q80, Q$Q50, Q$Q74, Q$Q86, stringsAsFactors = FALSE)
...
SoMi <- data.frame(Q$id, Q$Q8, Q$Q9, Q$Q10, Q$Q11, Q$Q12, Q$Q13, Q$Q14, Q$Q15, Q$Q16, Q$Q17, Q$Q18, Q$Q19, stringsAsFactors = FALSE)
...
## Creating a column for the first player's reports.
Data <-
  Data %>% 
