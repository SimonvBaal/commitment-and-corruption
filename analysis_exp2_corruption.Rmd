...
RawData <- read_csv("./raw-data/raw-data_exp2_corruption.csv")
```
...
densityPlotExp2 <- 
  ...
  theme_light() +
  facet_grid( ~ Effort)
ggsave("./figures/density-plot_exp2.png", width = w*1.5, height = 1.1*h)
...
ggsave("./figures/double-plot_exp2.png", width = w, height = 1.1*h)
 DoublePlot
...
ReportPlot <-
  ggplot(Figdf3, aes(x = Effort, y = Reports, colour = Rewards)) +
  geom_line(aes(group = Rewards),
            size = 0.8,
            position = position_dodge(width = .1)) +
  geom_point(aes(group = Rewards),
             size = 2,
             position = position_dodge(width = .1)) +
  geom_errorbar(
    aes(ymin = Reports - SE, ymax = Reports + SE),
    width = .1,
    size = 0.8,
    position = position_dodge(width = .1)
  ) +
  ...
  ggsave("./figures/report-plot_exp2.png", width = w, height = 1.1 * h)
ReportPlot
...
RewardPairs <- 
  pairs(DoubleEmm, adjust = "fdr")

RewardPairs2 <- 
  pairs(DoubleEmm, adjust = "fdr", reverse = T)
...
DoubleGLMMExp2Plot <-
  mixed(
    Double ~ Effort * Rewards + zcFirstMoverReport + (Effort + Rewards + zcFirstMoverReport |
                                                        ParticipantId),
    family = binomial(link = "logit"),
    method = "LRT",
    data = SecondMoverDataExp2 %>%
...
InteractionPlotsExp2 <-
  ggarrange(reportInteractionPlotExp2, doubleInteractionPlotExp2,
            nrow = 2)
ggsave(
  "./figures/interaction-plots_Exp2.png",
  height = h * 1,
  width = w * 1.3
)
